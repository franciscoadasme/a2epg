    ...
    ui(new Ui::APZoomWidget)
{
    ...
    QList<Qt::Orientation> orientations = QList<Qt::Orientation>()
...
{
    QToolButton *zoomButton = orientation == Qt::Horizontal
                              ? ui->hZoomOutButton
                              : ui->vZoomOutButton;
...
void APZoomWidget::paintEvent(QPaintEvent *)
{
    ...
    QLinearGradient gradient = QLinearGradient(innerRect.topLeft(),
...
void APZoomWidget::updateLocation()
{
    ...
    _animator->setStartValue(QPoint(_hotspot.x(),
                                    parentWidget()->rect().bottom()));
    _animator->setEndValue(QPoint(_hotspot.x(),
                                  parentWidget()->rect().bottom() - height() - 8));
}
