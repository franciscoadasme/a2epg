...
void EPSProfile::addSegment(EPSegment *segment, bool silent)
{
    ...
    insertSegmentInList(segment,
                        _segmentsGroupedByType.value(segment->type()->id()));
    connect(segment, SIGNAL(typeDidChanged(int)),
            SLOT(segmentDidChangeItsType(int)));
    connect(segment, SIGNAL(endsDidChanged()),
            SLOT(segmentDidChangeItsEnds()));
    connect(segment, SIGNAL(typeRestored(int)),
            SLOT(restoredSegmentType(int)));
...
void EPSProfile::addSegment(int type,
                            uint start,
                            uint end,
                            QString comments,
                            bool silent)
...
int EPSProfile::countOfEmptySpaces()
{
    ...
    if (EPSignalsController::activeSignal()->numberOfPoints()
        - segments.last()->end() > 5)
        emptySpaces++;
...
QVariant EPSProfile::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::DisplayRole) {
        ...
        case LengthColumn:
            return tr("%1s")
                    .arg(transformNumberOfPointsToSeconds(segment->length()));
        }
...
void EPSProfile::fillGaps()
{
    ...
    if (last->end() < EPSignalsController::activeSignal()->numberOfPoints() - 1)
        last->setEnd(EPSignalsController::activeSignal()->numberOfPoints() - 1,
                     true);
}
...
QList<EPSegment *> EPSProfile::findCollisionsForSegment(EPSegment *segment)
{
    ...
    uint start = segment->haveTranscientValues()
                 ? segment->transcientStart()
                 : segment->start();
    uint end = segment->haveTranscientValues()
...
void EPSProfile::fixInnerCollisions()
{
    ...
    for (int i = 0; i < count() - 1; i++) {
        ...
        /* we need to iterate through all segments again, since there are long
         * segments that still could be overlapping with the current one */
        for (int j = 0; j < count(); j++) {
            ...
            int mostReliableType = APUtils::mostReliableType(segment->type()->id(),
                                                             otherSegment->type()->id());
            ...
            if (isCurrentSegmentMoreReliable) {
                ...
                if (contains) {
                    ...
                    i--; j--;
                    if (i < 0) {
                        // case when a colision is found at the beginning
                        i = 0;
                    }
                } else {
                    ...
                    if (!otherSegment->satisfyDurationRestriction()) {
                        // if otherSegment is shorter than required duration,
                        // remove it
                ...
                if (contains) {
                    EPSegment *rightestSegment = new EPSegment(segment->type()->id(),
                                                               otherSegment->end() + 1,
                                                               segment->end());
                    ...
                    if (!rightestSegment->satisfyDurationRestriction()) {
                        // if rightestSegment (new) is shorter than required
                        // duration, remove it
                        EPSegment *next = (EPSegment *)objectAt(
                                              indexOfObject(rightestSegment) + 1);
                        if (next) {
...
void EPSProfile::fixCollisionForSegment(EPSegment *segment)
{
    ...
    if (collidedSegments.isEmpty()) {
        editHandler->saveAction(segmentRecentlyAdded
                                ? APEditHandler::Create
                                : APEditHandler::Update,
                                segmentRecentlyAdded
                                ? segment->serialize()
                                : segment->lastChangeInfo());
    ...
    foreach (EPSegment *collidedSegment, collidedSegments) {
        ...
        if (contains) {
            ...
            editHandler->saveAction(APEditHandler::Remove,
                                    collidedSegment->serialize());
        } else if (contained) {
            // if they are equals, just remove new segment...
            if (collidedSegment->type() == segment->type()) {
                ...
                editHandler->saveAction(APEditHandler::Create,
                                        rightPart->serialize());

                collidedSegment->setEnd(segment->start() - 1, true);
                editHandler->saveAction(APEditHandler::Update,
                                        collidedSegment->lastChangeInfo());
            }
            ...
            if (collidedSegment->type() == segment->type()) {
                segment->setEnd(collidedSegment->end(), true);
                // if it was just added, wait for collisions fixes
                if (!segmentRecentlyAdded)
                    editHandler->saveAction(APEditHandler::Update,
                                            segment->lastChangeInfo());
                ...
                editHandler->saveAction(APEditHandler::Update,
                   ...
                   && collidedSegment->start() <= segment->start()) {
            // left overlap
            if (collidedSegment->type() == segment->type()) {
                segment->setStart(collidedSegment->start(), true);
                // if it was just added, wait for collisions fixes
...
QVariant EPSProfile::headerData(int section,
...
void EPSProfile::mergeSameSegments()
{
    for (int i = 0; i < count() - 1; i++) {
        ...
        if (other->start() - segment->end() < 200
...
void EPSProfile::removeAllSegmentsOfType(int type)
{
    ...
    Q_ASSERT_X(APSegmentTypesController::typeIds().contains(type),
               "EPSProfile::removeAllSegmentsOfType",
               "type not valid");
...
void EPSProfile::restoredSegmentType(int oldType)
{
    ...
    dataChanged(index(indexOfObject(segment), 0),
                index(indexOfObject(segment), 0));
}
...
void EPSProfile::segmentDidChangeItsType(int oldType)
{
    ...
    editHandler->beginGroup(); {
        ...
        EPSegment *next = indexOfSegment < count() - 1
                          ? (EPSegment *)objectAt(indexOfSegment + 1)
                          : NULL;
        EPSegment *prev = indexOfSegment > 0
                          ? (EPSegment *)objectAt(indexOfSegment - 1)
                          : NULL;
        ...
        if (next && prev
            ...
            qDebug() << "EPSProfile::segmentDidChangeItsType() => prev,"
                     << "segment and next have same type";

            prev->setEnd(next->end(), true);
            editHandler->saveAction(APEditHandler::Update,
                                    prev->lastChangeInfo());
            dataChanged(index(indexOfObject(prev), 0),
                        index(indexOfObject(prev), 0));
            ...
            qDebug() << "EPSProfile::segmentDidChangeItsType() =>"
                     << "next and segment have same type";

            next->setStart(segment->start(), true);
            editHandler->saveAction(APEditHandler::Update,
                                    next->lastChangeInfo());
            dataChanged(index(indexOfObject(next), 0),
                        index(indexOfObject(next), 0));
            ...
            qDebug() << "EPSProfile::segmentDidChangeItsType() =>"
                     << "prev and segment have same type";
...
QList<EPSegment *> EPSProfile::segmentsInRangeOfType(uint start,
                                                     uint end,
                                                     int typeId)
{
    QList<EPSegment *> segments = QList<EPSegment *>();
    foreach (EPSegment *segment, segmentsOfType(typeId)) {
        uint segmentStart = segment->haveTranscientValues()
                            ? segment->transcientStart()
                            : segment->start();
        uint segmentEnd = segment->haveTranscientValues()
                          ? segment->transcientEnd()
                          : segment->end();
