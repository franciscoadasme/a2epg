...
protected:
	...
	QList<float> peaksInsideWindow(Direction direction, bool returnValues = true);
	float getFrecuency(Direction direction = Up);
	float getAmplitude();

	virtual bool isAllowedPoint() { Q_ASSERT_X(false, "APSeeker", "Did not call overridden function"); return false; }
