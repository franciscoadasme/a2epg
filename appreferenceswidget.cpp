    ...
    ui(new Ui::APPreferencesWidget)
{
    ...
    QSettings settings;
    ui->openPanoramicCheckBox->setChecked(
                settings.value(OpenPanoramicOnSignalChangeKey, false).toBool());
    ui->askWhenDeletingSegmentCheckBox->setChecked(
                settings.value(AskWhenDeletingSegmentsKey, true).toBool());
    ui->liveSegmentResizingCheckBox->setChecked(
                settings.value(LiveSegmentResizingKey, false).toBool());
    ui->openInfoCheckBox->setChecked(
                settings.value(OpenInfoOnSignalChangeKey, false).toBool());
    ui->pdMaximumDurationSpinBox->setValue(
                settings.value(PdMaximumDurationKey, 10).toFloat());
    ui->fillGapsCheckBox->setChecked(
                settings.value(FillGapsDuringScanKey, true).toBool());

    ui->segmentTypesTableView->setModel(
                APSegmentTypesController::shared()->arrangedObjects());
    APSegmentTypesController::shared()->setSelectionModel(
                ui->segmentTypesTableView->selectionModel());
}
...
{
    QSettings settings;
    settings.setValue(OpenPanoramicOnSignalChangeKey,
                      ui->openPanoramicCheckBox->isChecked());
}

void APPreferencesWidget::on_askWhenDeletingSegmentCheckBox_clicked()
{
    QSettings settings;
    settings.setValue(AskWhenDeletingSegmentsKey,
                      ui->askWhenDeletingSegmentCheckBox->isChecked());
}

void APPreferencesWidget::on_liveSegmentResizingCheckBox_clicked()
{
    QSettings settings;
    settings.setValue(LiveSegmentResizingKey,
                      ui->liveSegmentResizingCheckBox->isChecked());
}

void APPreferencesWidget::on_openInfoCheckBox_clicked()
{
    QSettings settings;
    settings.setValue(OpenInfoOnSignalChangeKey,
                      ui->openInfoCheckBox->isChecked());
}

void APPreferencesWidget::on_segmentTypesTableView_activated(const QModelIndex &index)
{
    // only edit color, which is at fist column... anything else, avoid it
    if (index.column() != 0) return;

    APSegmentTypesController *controller = APSegmentTypesController::shared();
    QColor currentColor = controller->data(index, Qt::DecorationRole).value<QColor>();
    QColor color = QColorDialog::getColor(currentColor, this);
    if (color.isValid())
        controller->setData(index, color, Qt::EditRole);
}
...
void APPreferencesWidget::on_createSegmentTypeButton_clicked()
{
    ...
    APSegmentTypesController::shared()->addSegmentType(text,
                                                       APUtils::randomColor());
}
