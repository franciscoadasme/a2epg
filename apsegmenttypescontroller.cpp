...
QList<APSegmentType *> APSegmentTypesController::defaultSegmentTypes()
{
	...
	foreach (SegmentType type, APUtils::segmentTypes()) {
		types << new APSegmentType(type, APUtils::stringWithSegmentType(type), APUtils::colorForSegmentType(type));
	}
	return types;
}
...
QVariant APSegmentTypesController::headerData(int section, Qt::Orientation orientation, int role) const
{
...
void APSegmentTypesController::loadSegmentTypes()
{
	...
	if (settings.contains(SegmentTypesPersistenceKey)) {
		QString representation = settings.value(SegmentTypesPersistenceKey).toString();
...
bool APSegmentTypesController::setData(const QModelIndex &index, const QVariant &value, int role)
{
