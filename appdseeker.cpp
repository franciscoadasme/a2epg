...
bool APPdSeeker::areInnerPointsLowerThanEnds(int left,
                                             int right,
                                             QList<float> topPeaks)
{
    ...
    for (int i = 0; i < topPeaks.count(); i++) {
        ...
        float peakIndex = topPeaks[i];

        float slopeToHighest = APUtils::slope(QPointF(peakIndex - 10, highest),
                                              QPointF(peakIndex, points[peakIndex]))
                               * 100;
        if (highest == right) slopeToHighest *= -1;
        float slopeToLowest = APUtils::slope(QPointF(peakIndex - 10, lowest),
                                             QPointF(peakIndex, points[peakIndex]))
                              * 100;

//        writeLog(tr("%1\t%2\t%3\t%4\t%5").arg(left).arg(right).arg(peakIndice)
//                 .arg(slopeToHighest).arg(slopeToLowest));
...
//    qDebug() << "APPdSeeker::areSurroundingsHigher =>"
...
QList<QPair<int, int> > APPdSeeker::chooseLongestSegments(
        QList<QPair<int, int> > foundSegments)
{
    ...
    for (int i = 0; i < foundSegments.count(); i++) {
        ...
        if (i == foundSegments.count() - 1 // at the last one
            ...
            for (int j = 1; j < segmentsWithSharedEnds.count(); j++) {
                int longestLength = longestSegment.second
                                    - longestSegment.first + 1;
...
QList<QPair<int, int> > APPdSeeker::findSunkenSegments(
        QList<QPair<int, Direction> > slopeIndices)
{
    ...
    for (int i = 0; i < slopeIndices.count() - 1; i++) {
        ...
        // should start with upward peak
        if (points[slopeIndex.first] < points[slopeIndex.first + 1]) continue;
        ...
        if (i % (slopeIndices.count() / 100) == 0) {
            float total = (float)slopeIndices.count();
            emit progressDidChange(qRound(i * 100.0 / total * .2) + 20);
        }
    }
...
QList<QPair<int, int> > APPdSeeker::filterSunkenSegments(
        QList<QPair<int, int> > proposedSunkenSegments)
{
    ...
    foreach (segment, proposedSunkenSegments) {
        ...
        QList<float> topPeaksInsideSegment = peakIndicesInRange(Up,
                                                                segment.first,
                                                                segment.second);
        ...
        if (points[segment.first] < points[segment.second]
            && areInnerPointsLowerThanEnds(segment.first,
                                           segment.second,
                                           topPeaksInsideSegment)
            && satisfyInnerRestrictions(segment.first,
                                        segment.second,
                                        topPeaksInsideSegment)) {
            segments << segment;
        }
        ...
        if (++i % (proposedSunkenSegments.count() / 100) == 0) {
            float total = (float)proposedSunkenSegments.count();
            int progress = qRound(i * 100.0 / total * .6);
            emit progressDidChange(progress + 40);
            emit workThrowsMessage(tr("Filtering found segments... %1%")
                                   .arg(progress));
        }
    }
...
bool APPdSeeker::satisfyInnerDeviation(QList<float> topPeaks)
{
    ...
    for (int i = 0; i < topPeaks.count() - 1; i++) {
        ...
        slopes << APUtils::slope(QPointF(peakIndice, points[peakIndice]),
                                 QPointF(nextPeakIndice, points[nextPeakIndice]))
                  * 100;
    }
    ...
    if (slopeDeviation > 2) {
//        qDebug().nospace() << "APPdSeeker::satisfyInnerDeviation() => "
//                           << "slope deviation greater than 2";
        return false;
    }
...
bool APPdSeeker::satisfyInnerPeakHeightRestrictions(int left,
                                                    int right,
                                                    QList<float> topPeaks)
{
...
//        writeLog(tr("%1\t%2\t%3\t%4\t%5").arg(left).arg(right)
//                 .arg(peaks.count()*.25).arg(countOfTinyPeaks).arg(height));
...
{
//    qDebug().nospace() << "APPdSeeker::satisfyInnerRestrictions("
//                       << left << ", " << right << ")";
//    EPSignalWidget *epsw = MainWindow::instance()->epsignalWidget();
//    epsw->pushDebugPoints(topPeaks, Qt::red);
//    epsw->pushDebugPoints(bottomPeaks, Qt::blue);
...
void APPdSeeker::seek()
{
    ...
    for (int i = 0; i < indices.length(); i++) {
        ...
        for (int offset = 1; offset < 4 && i + offset < indices.count(); offset++) {
            ...
            /* Pd beginning (downward slope) should be calculated only between
             * i and i + 1, since Pd always start with a steep slope */
            ...
            if (qAbs(height) > param("minimumHeight")) {
                ...
                for (int j = slopes.count() - 1; j >= 0; j--) {
                    ...
                    if (index - slope.left > transformSecondsToNumberOfPoints(1))
                ...
                if (shouldBeConsidered)
                    slopes << slopeWithLeftRightHeightAndDirection(index,
                                                                   nextIndex,
                                                                   qAbs(height),
                                                                   direction);
            }
        ...
        if (i % (indices.count() / 100) == 0) {
            float total = (float)indices.count();
            emit progressDidChange(qRound(i * 100.0 / total * .2));
        }
    }
    ...
    for (int i = 0; i < slopes.count() - 1; i++) {
        ...
        for (int j = i + 1; j < slopes.count(); j++) {
            ...
            if (nextSlope.direction == Down)
                continue; // must end with upward slope
            else if (nextSlope.right - slope.left > param("maximumDuration"))
                break;
            else if (nextSlope.right - slope.left < param("minimumDuration"))
                continue;

//            qDebug().nospace() << "APPdSeeker::seek() => left=" << slope.left
//                               << ", right=" << nextSlope.right
//                               << ", areSurroundingsHigher="
//                               << areSurroundingsHigher(slope, nextSlope);
...
//            float total = (float)slopes.count();
//            emit progressDidChange(qRound(i * 100.0 / total * .2) + 20);
//        }
    }
}
