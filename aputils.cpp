...
QList<SegmentType> APUtils::_segmentTypes =
        QList<SegmentType>() << Np << C << Pd << Pd1 << Pd2 << Pd3 << E1 << E2
                             << F << G;
QList<QString> APUtils::_segmentTypeNames =
        QList<QString>() << QObject::tr("Np") << QObject::tr("C")
                         << QString("Pd") << QString("Pd1") << QString("Pd2")
                         << QString("Pd3") << QString("E1") << QString("E2")
                         << QString("F") << QString("G");
QList<SegmentType> APUtils::segmentTypesOrderedByConfidence = QList<SegmentType>()
                                                              << Np << Pd << G << F << E1 << C;
QList<int> APUtils::durationOfSegmentTypes =
        QList<int>() << 5 << 4 << 2 << 2 << 2 << 2 << 15 << 15 << 15 << 20;
...
void APUtils::paintSignalUsingPainterWithRect(EPSignal *signal,
                                              QPainter *painter,
                                              QRect bounds)
{
    painter->save();
    ...
    for (uint i = 1; i < numberOfPointsToDraw; i++) {
        ...
        // be aware that we multiply by -1 to flip out the signal, so positive
        // values point to top
...
void APUtils::paintVerticalGridInRect(QPainter *painter, QRect rect)
{
    ...
    for (uint i = offset;
         i < offset + rect.width() / viewport->horizontalMultiplier();
         ++i) {
        if (i % pointPerSecond == 0) {

            QLineF line = QLineF((i - offset) * viewport->horizontalMultiplier(),
                                 1,
                                 (i - offset) * viewport->horizontalMultiplier(),
                                 rect.height());
...
QList<float> APUtils::peaks(QList<float> list,
                            Direction dir,
                            int windowSize,
                            bool returnValues,
                            bool checkSignificance)
{
    //	QTime t;
    //	t.start();
    ...
    for (int i = 1; i < list.length() - 1; i++) {
        ...
        for (int offset = 1; offset <= windowSize / 2; offset++) {
            ...
            /* compare which each sibling and check if it a peak... if not,
             * break the loops */
            QList<int> siblingIndices = QList<int>() << leftSiblingIndex
                                                     << rightSiblingIndex;
            foreach (int siblingIndex, siblingIndices) {
                ...
                bool notLeadingCopy = list[i] == list[siblingIndex]
                                      && i > siblingIndex;
                ...
                if (notTopPeak
                    || notBottomPeak
                    || notLeadingCopy
                    || (checkSignificance && notSignificantPeak)) {
...
QString APUtils::runSaveDialog(int extensionFlags, QString defaultExtension)
{
    ...
    QString selectedDir = settings.value(LastLocationVisitedKey,
    ...
    if (fileName.isEmpty()) return "";
        ...
        // append extension based on selected filter
        if (fileInfo.suffix().isEmpty() || !extensions.contains(fileInfo.suffix())) {
