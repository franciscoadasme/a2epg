...
bool EPSignalWidget::isOversized()
{
    ...
    return EPSignalsController::activeSignal()->hasMorePointsThan(
                numberOfPointsThatCanBeDrawn);
}
...
void EPSignalWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (isInAddingMode) {
        if (temporalSegment) {
            /* Calculate distance from hotspot to each end to check which end
             * should be modified */
...
void EPSignalWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    if (isInAddingMode) {
        ...
        if (temporalSegment) {
            ...
            APSegmentDialog *dialog = APSegmentDialog::instance();
            if (temporalSegment->type()->id() != APNotFound
                || dialog->runForSegment(temporalSegment)== QDialog::Accepted) {
                EPSignalsController::activeSignal()->profile()
                        ->addSegment(temporalSegment);
                MainWindow::instance()->epsprofileWidget()
                        ->setFocusedSegment(temporalSegment);
            } else {
...
void EPSignalWidget::paintDebugPoints(QPainter *painter)
{
    ...
    float hMultiplier = viewport->horizontalMultiplier();
    float vMultiplier = viewport->verticalMultiplier();
    ...
    for (int i = 0; i < _debugPoints.length(); i++) {
        ...
        foreach (int point, pair.first) {
            int x = (point- offset) * hMultiplier;
    ...
    for (int i = 0; i < _debugLines.count(); i++) {
        ...
        float x1 = (pair.first.x1() - offset) * hMultiplier;
        float x2 = (pair.first.x2() - offset) * hMultiplier;
        float y1 = pair.first.y1() * vMultiplier;
        float y2 = pair.first.y2() * vMultiplier;
...
void EPSignalWidget::paintEvent(QPaintEvent *paintEvent)
{
    ...
    painter->save(); {
        ...
        APUtils::paintSignalUsingPainterWithRect(
                    EPSignalsController::activeSignal(),
                    painter,
                    rect());
    } painter->restore();
...
void EPSignalWidget::paintVerticalRuler(QPainter *painter)
{
    ...
    painter->drawLine(VerticalHeaderWidth + 1, 0,
                      VerticalHeaderWidth + 1, height());
...
void EPSignalWidget::paintMouseLocationInfo(QPainter *painter)
{
    ...
    int x = (lastMousePosition - viewport->offset())
            * viewport->horizontalMultiplier();
    QRect frame = QRect(x + 8, 8, 128, 16);

    // paint line only when start location haven't been set
    if (!temporalSegment || isInTrackingMode) {
        painter->drawLine(x, 0, x, height());
    }
    painter->drawText(frame,
                      Qt::AlignVCenter | Qt::AlignLeading,
                      temporalSegment == NULL
                      ? QString::number(transformNumberOfPointsToSeconds(lastMousePosition))
...
void EPSignalWidget::paintHorizontalRuler(QPainter *painter)
{
    ...
    painter->setPen(Qt::NoPen);
    painter->drawRect(0, height() - HorizontalRulerHeight,
                      width(), height() - HorizontalRulerHeight);
    ...
    // draw numbers
    painter->setPen(Qt::gray);
    for (uint i = offset; i < offset + width() / hMultiplier; ++i) {
        if (i > 0 && i % pointsPerSecond == 0) {
            if (i/100 > 999
                && hMultiplier < 0.6
                && i/100 % 5 != 0)
                continue;
            if ((i/100 > 99 && i/100 <= 999)
                && hMultiplier < 0.3
                && i/100 % 2 != 0)
                continue;

            painter->drawText(QRect((i - offset) * hMultiplier - 20,
