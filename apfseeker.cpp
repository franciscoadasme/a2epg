...
//	}
	...
	float amplitude = APUtils::getAverage(topPeakValues) - APUtils::getAverage(bottomPeakValues);
	float normalizedTopPeaksDeviation = APUtils::getDeviation(topPeakValues, APUtils::getAverage(topPeakValues)) / amplitude;
	float normalizedBottomPeaksDeviation = APUtils::getDeviation(bottomPeakValues, APUtils::getAverage(bottomPeakValues)) / amplitude;

//	writeLog(tr("%1\t%2\t%3\t%4\t%5\t%6\t%7").arg(left()).arg(right()).arg(topFrecuency).arg(bottomFrecuency).arg(amplitude)
//			 .arg(normalizedTopPeaksDeviation).arg(normalizedBottomPeaksDeviation));

	bool satisfyFrecuencyRestriction = topFrecuency > param("minimumTopFrecuency")
	...
	bool satisfyPeakDeviationRestriction = normalizedTopPeaksDeviation > param("minimumPeakDeviation")
