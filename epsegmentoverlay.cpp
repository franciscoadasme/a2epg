...
void EPSegmentOverlay::forceToUpdateGeometry()
{
	...
	int start = _segment->haveTranscientValues() ? _segment->transcientStart() : _segment->start();
...
void EPSegmentOverlay::paintEvent(QPaintEvent *)
{
	...
	if (_segment->isCommented()) {
		...
		painter->drawText(textRect, Qt::AlignLeading | Qt::AlignTop | Qt::TextWordWrap,
						  _segment->comments());
	}
...
void EPSegmentOverlay::setSegment(EPSegment *segment)
{
	...
	connect(_segment, SIGNAL(endsDidChanged()), SLOT(forceToUpdateGeometry()));
	connect(_segment, SIGNAL(transcientEndsDidChanged()), SLOT(forceToUpdateGeometry()));
	connect(_segment, SIGNAL(typeDidChanged()), SLOT(update()));
	connect(_segment, SIGNAL(commentsDidChanged()), SLOT(update()));
