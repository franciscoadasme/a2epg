		...
		if (i == _proposedSegments.count() - 1 // at the last one
			...
			if (segmentsWithSharedEnds.count() > 1) {
				...
				for (int j = 1; j < segmentsWithSharedEnds.count(); j++) {
					int shortestLength = shortestSegment->second - shortestSegment->first + 1;
...
QList<Slope> APNewPdSeeker::findSteepestSlopes(QList<float> indices)
{
	...
	for (int i = 0; i < indices.count(); i++) {
		...
		for (int offset = 1; offset < 6 && i + offset < indices.count(); offset++) {
            ...
            /* Pd beginning (downward slope) should be calculated only between i and i + 3,
				since Pd always start with a steep slope */
            if (direction == Down && offset > 3) break;

			/* They cannot be to far away */
			if (nextIndex - index > transformSecondsToNumberOfPoints(direction == Down ? .35 : .5)) break;

			/* Pd start slope should measured between top and bottom peaks (not between top and top) */
            float isTopPeak = points[index] > points[index + 1] || points[index] > points[index - 1];
			...
			if (qAbs(height) > param("minimumHeight")) {
				...
				for (int j = slopes.count() - 1; j >= 0; j--) {
					...
					if (index - slope.left > transformSecondsToNumberOfPoints(.5)) break; /* not beyond 100 points */
...
//			MainWindow::instance()->epsignalWidget()->pushDebugLine(slope.left, points[slope.left],
//																	slope.right, points[slope.right]);
//			MainWindow::instance()->epsignalWidget()->pushDebugLine(other.left, points[other.left],
//																	other.right, points[other.right]);
            ...
            if (qAbs(slope.height - other.height) > averageHeight * /*.65*/ .75) continue;
			...
			if (slopeValue * 100 < -0.1) continue;
			...
			for (int i = 0; i < transformSecondsToNumberOfPoints(1); i++) {
				float sibling = points[slope.left - transformSecondsToNumberOfPoints(.25) - i];
			...
			for (int pos = slope.right + 1; pos < other.left - 1; pos++) { innerPoints << points[pos]; }
...
bool APNewPdSeeker::areMostInnerPeaksBeneathRoof(Slope start, Slope end, QList<float> peaks,
...
//		MainWindow::instance()->epsignalWidget()->pushDebugLine(index, points[index], index, (m * (index - x) + y), QColor(0, 0, 255, 63));
...
bool APNewPdSeeker::satisfyStrongVariantRestrictions(Slope start, Slope end, QList<float> topPeaks, QList<float> bottomPeaks)
{
...
bool APNewPdSeeker::satisfyWeakVariantRestrictions(Slope start, Slope end, QList<float> topPeaks, QList<float> bottomPeaks)
{
    if (!areMostInnerPeaksBeneathRoof(start, end, topPeaks, .1, .95)) return false;

    float minimumValue = points[start.left] - start.height * .6,
            lowerThreshold = points[start.right];

//    MainWindow::instance()->epsignalWidget()->pushDebugLine(start.right + 1, minimumValue, end.left - 1, minimumValue, Qt::darkCyan);
//    MainWindow::instance()->epsignalWidget()->pushDebugLine(start.right + 1, lowerThreshold, end.left - 1, lowerThreshold, Qt::darkRed);
...
//        MainWindow::instance()->epsignalWidget()->pushDebugLine(index, value, index, minimumValue, Qt::cyan);
    }
