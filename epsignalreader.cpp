...
EPSignalReader::EPSignalReader(QList<QStringList> groupedFilePaths,
                               QObject *parent) :
...
void EPSignalReader::dispatchReader(QList<QStringList> groupedFilePaths,
                                    QObject *target,
                                    const char *callback)
{
...
bool EPSignalReader::readEPG()
{
    ...
    if (_xmlReader->readNextStartElement()) {
        if (_xmlReader->name() != "epsignal"
            ...
            errorMessage = tr("Parse error: either file does not follow epg "
                              "format or it's not version 1.0.");
            return false;
        }
