...
void APSeekerWidget::dispatchSeekersForTypes(QList<SegmentType> types,
                                             bool shouldFillGaps)
{
    ...
    foreach (SegmentType type, seekerWidget->_segmentTypesToSeekFor) {
        APSeeker *seeker = APSeeker::seekerForTypeAndSignal(type,
                                                            signal,
                                                            seekerWidget);
        ...
        seekerWidget->ui->grid->addWidget(
                    new QLabel(APUtils::stringWithSegmentType(type)), row, 0);
...
void APSeekerWidget::showInfoMessage()
{
    ...
    foreach (APSeeker *seeker, _seekers) {
        int count = profile->segmentsOfType(seeker->type()).count();
        if (count > 0) {
            numberOfFoundSegments += count;
            stream << qSetFieldWidth(2 +2)
                   << left
                   << APUtils::stringWithSegmentType(seeker->type())
                   << qSetFieldWidth(11 +2)
                   << tr("%1 segment%2").arg(count).arg(count > 1 ? "s" : "")
                   << reset
                   << profile->totalDurationOfSegmentType(seeker->type())
                   << "s"
                   << "<br/>";
        } else {
            stream << qSetFieldWidth(2 +2)
