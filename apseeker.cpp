...
QList<float> *APSeeker::peakIndices(Direction direction)
{
    ...
    if (indices->isEmpty()) {
        ...
        indices->append(APUtils::peaks(points, direction,
                                       param("peakWindowSize"), false));
...
void APSeeker::run()
{
    ...
    emit workDidEnd();
    emit workDidEnd(true, object(),
                    _proposedSegments.isEmpty()
                    ? "No segments found"
                    : QString("%1 segment(s) found").arg(_proposedSegments.count()));
}

APSeeker *APSeeker::seekerForTypeAndSignal(SegmentType type,
                                           EPSignal *signal,
                                           QObject *parent)
{
    switch (type) {
...
void APSeeker::setUp()
{
    ...
    if (param("needsLog")) {
        ...
        Q_ASSERT_X(_logFile->open(QFile::WriteOnly | QFile::Text),
                   "APSeeker::setUp()",
