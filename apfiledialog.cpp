  ...
  const QStringList &filePaths)
{
  ...
  foreach (QString filePath, filePaths) {
    if (APFileInfo(filePath).isNumbered()) {
      ...
      dialog.setText(
        tr("One or more files appear to be part of a larger recording."));
      dialog.setInformativeText(
        tr("Do you want to load existing files that belong to the same "
           "record as well?"));
...
QList<QStringList> APFileDialog::getGroupedOpenFileNames(
  QWidget *parent,
