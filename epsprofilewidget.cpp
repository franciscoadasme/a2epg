...
void EPSProfileWidget::currentSignalDidChange(EPSignal *signal)
{
    ...
    if (_profile) {
        this->connect(_profile, SIGNAL(segmentsDidChange()),
                      SLOT(segmentsDidChange()));
        updateMaximumHeight();
    } else
        update();
}
...
void EPSProfileWidget::dragEvent(QMouseEvent *event)
{
    ...
    /* if hotspot is at any widget's edges, move offset until either limit has
     * reached */ {
        if (event->x() < 5 && viewport->offset() > 0) {
            // if offset has reached its ends, do nothing
            viewport->setOffset(viewport->offset() - abs(event->x()));
            hotspot = viewport->transform(5);
        } else if (event->x() > width() - 5
                   && !viewport->hasBeenReachedOffsetLimit()) {
    ...
    /* check minimum length*/ {
        int proposedLength = draggingEnd == Left
                             ? focusedSegment()->end() - hotspot
                             : hotspot - focusedSegment()->start();
    ...
    /* update focused segment ends */ {
        ...
        if (draggingEnd == Left) {
            ...
            if (settings.value(LiveSegmentResizingKey, false).toBool())
                focusedSegment()->setEnd(hotspot > numberOfPoints
                                         ? numberOfPoints
                                         : hotspot);
            else
                focusedSegment()->setTranscientEnd(hotspot > numberOfPoints
                                                   ? numberOfPoints
                                                   : hotspot);
        }
    }
...
bool EPSProfileWidget::isMouseOverAnySegmentEnds(QPoint position)
{
    ...
    bool isAtLeftEnd = abs(hotspot - segmentUnderCursor->start())
                       < 4 / viewport->horizontalMultiplier();
    bool isAtRightEnd = abs(hotspot - segmentUnderCursor->end())
                        < 4 / viewport->horizontalMultiplier();
...
{
    bool mouseHasMovedIntoHeader = event->x() < HorizontalHeaderWidth
                                   && event->x() > 0;
...
void EPSProfileWidget::mousePressEvent(QMouseEvent *event)
{
    ...
    if (isMouseOverAnySegmentEnds(event->pos())) {
        ...
        QSettings settings;
        if (!settings.value(LiveSegmentResizingKey, false).toBool())
            // in live resizing, we don't need transcient values
            focusedSegment()->setTranscientRange(focusedSegment()->start(),
                                                 focusedSegment()->end());
    }
    isMouseDown = true;
}
...
void EPSProfileWidget::mouseReleaseEvent(QMouseEvent *event)
{
    if (isDragging) {
        ...
        QSettings settings;
        if (!settings.value(LiveSegmentResizingKey, false).toBool()) {
            // in live resizing, we don't need to ask about collisions
            QList<EPSegment *> collidedSegments = focusedSegment()->findCollisions();
            bool shouldAcceptChanges = true;
            if (!collidedSegments.isEmpty()) {
                int ret = APSegmentDialog::askAboutCollisions(focusedSegment(),
                                                              collidedSegments);
                shouldAcceptChanges = ret == QMessageBox::Save;
            }
            ...
            update();
            MainWindow::instance()->epsignalWidget()->overlay()
...
void EPSProfileWidget::paintDescriptionForSegment(QPainter *painter,
                                                  EPSegment *segment,
                                                  QRectF rect)
{
    /* define aligment */
...
void EPSProfileWidget::paintEvent(QPaintEvent *event)
{
    ...
    APUtils::paintVerticalGridInRect(painter,
                                     rect().adjusted(0, 0, 0, innerVerticalOffset));
...
void EPSProfileWidget::paintHeaderForTypes(QPainter *painter, QList<int> types)
{
    ...
    painter->setBrush(QColor(245, 245, 245,
                             shouldPaintTransparent ? 128 : 232)); // alpha 85%
    ...
    painter->setPen(QColor(216, 216, 216,
                           shouldPaintTransparent ? 128 : 232)); // alpha 85%
    painter->drawLine(HorizontalHeaderWidth + 1, 0,
                      HorizontalHeaderWidth + 1, maximumHeight());
    ...
    foreach (int type, types) {
        ...
        painter->drawText(QRect(0, verticalOffset - 10,
                                HorizontalHeaderWidth, 20),
...
void EPSProfileWidget::paintHorizontalGrid(QPainter *painter)
{
    ...
    for (int verticalOffset = SegmentRowVerticalSpace;
         verticalOffset < height() + innerVerticalOffset;
         verticalOffset += SegmentRowVerticalSpace) {
        painter->drawLine(QPoint(0, verticalOffset),
                          QPoint(width(), verticalOffset));
    }

    painter->restore();
}

void EPSProfileWidget::paintSegment(QPainter *painter,
                                    EPSegment *segment,
                                    uint verticalOffset)
{
    painter->save();
    ...
    int start = segment->haveTranscientValues()
                ? segment->transcientStart()
                : segment->start();
    int end = segment->haveTranscientValues()
              ? segment->transcientEnd()
              : segment->end();

    float segmentLeft = (start - (int)viewport->offset())
                        * viewport->horizontalMultiplier();
    float segmentRight = (end - (int)viewport->offset())
                         * viewport->horizontalMultiplier();
...
void EPSProfileWidget::paintSegmentsOfType(QPainter *painter,
                                           int type,
                                           uint verticalOffset)
{
    painter->save();
    ...
    uint left = viewport->offset(),
         right = viewport->offset()
                 + viewport->numberOfPointsThatCanBeDrawnInWidth(width());
    QList<EPSegment *> segmentsToDraw =
            _profile->segmentsInRangeOfType(left, right, type);
...
EPSegment *EPSProfileWidget::segmentAtPosition(QPoint position)
{
    ...
    for (int i = 0; i < types.length(); ++i) {
        // find in which segment type line it is
        if (position.y() > segmentLineVerticalCenter - 10
            && position.y() < segmentLineVerticalCenter + 12) {
            // add 2 bottom padding to tracking purpose
...
void EPSProfileWidget::setPaintAttributesForSegment(QPainter *painter,
                                                    EPSegment *segment,
                                                    QRectF rect)
{
    ...
    QLinearGradient gradient = QLinearGradient(rect.x(), rect.y(),
    ...
    painter->setPen(shouldHighlight
                    ? segmentColor.darker(125)
                    : segmentColor.lighter(125));
}

void EPSProfileWidget::updateMaximumHeight()
{
    //	qDebug("EPSProfileWidget::updateMaximumHeight()");
    setMaximumHeight(SegmentRowVerticalSpace
                     * (_profile->currentTypes().length() + 1));
...
void EPSProfileWidget::wheelEvent(QWheelEvent *event)
{
    if (isOversized) {
        ...
        else if (offset > maximumHeight() - height()) {
            offset = maximumHeight() - height();
        }
