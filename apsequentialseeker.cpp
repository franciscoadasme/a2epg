...
float APSequentialSeeker::getAmplitude()
{
    ...
    return APUtils::getAverage(peakUpValues)
            - APUtils::getAverage(peakDownValues);
}

float APSequentialSeeker::getFrecuency(Direction direction)
{
    QList<float> topPeakValues = peaksInsideWindow(direction);
    return topPeakValues.length() * (transformSecondsToNumberOfPoints(1)
                                     / pointsInsideWindow.count());
}
...
void APSequentialSeeker::loadPointsInsideWindow()
{
    if (pointsInsideWindow.isEmpty()) {
        // in load window, we add a new element, so we initialize with
        // windowSize -1
...
QList<float> APSequentialSeeker::peaksInsideWindow(Direction direction,
                                                   bool returnValues)
{
    QList<float> *peakIndices = APSeeker::peakIndices(direction);

    QList<float> *peakIndicesInsideWindow = direction == Up
                                            ? &topPeakIndicesInsideWindow
                                            : &bottomPeakIndicesInsideWindow;
    int *peakIndicesOffset = direction == Up
                             ? &topPeakIndicesOffset
                             : &bottomPeakIndicesOffset;

    if (peakIndicesInsideWindow->isEmpty()) {
        for (int i = *peakIndicesOffset;
             i < peakIndices->length()&& peakIndices->at(i) <= right();
             i++) {
            peakIndicesInsideWindow->append(peakIndices->at(i));
        }
    } else {
        while (!peakIndicesInsideWindow->isEmpty()
               && peakIndicesInsideWindow->first() < left()) {
            peakIndicesInsideWindow->removeFirst();
            (*peakIndicesOffset)++;
        }

        int nextRightPeakIndex = *peakIndicesOffset
                                 + peakIndicesInsideWindow->count();
        while (nextRightPeakIndex < peakIndices->length()
               && peakIndices->at(nextRightPeakIndex) <= right()) {
...
void APSequentialSeeker::seek()
{
    ...
    while (walkable()) {
        ...
        if (_proposedSegments.isEmpty()
            || left() > (int)_proposedSegments.last()->second) {
            ...
            if (isAllowedPoint()) {
                ...
                if (didFindAnythingYet()
                    && proposedRangeSatisfyMinimumDuration(right() - 1)) {
        ...
        if (middle() % pointsInOnePercent == 0) {
            ...
            emit workThrowsMessage(tr("Processing %1%...")
                                   .arg(qRound(middle() * 100.0 / total)));
            emit progressDidChange(qRound(middle() * 100.0 / total));
        }
