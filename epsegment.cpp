...
QString EPSegment::description(bool verbose)
{
    return verbose ? tr("%1:%2-%3").arg(type()->name())
                     .arg(transformNumberOfPointsToSeconds(start()))
                     .arg(transformNumberOfPointsToSeconds(end()))
                   : tr("%1 to %2")
                     .arg(transformNumberOfPointsToSeconds(haveTranscientValues()
                                                           ? transcientStart()
                                                           : start()))
                     .arg(transformNumberOfPointsToSeconds(haveTranscientValues()
                                                           ? transcientEnd()
...
QList<EPSegment *> EPSegment::findCollisions()
{
    ...
    return alreadyAdded
            ? profile->findCollisionsForSegment(this)
            : profile->findCollisionsForProposedSegment(this);
}

bool EPSegment::haveTranscientValues()
{
    return (int)_transcientStart != APNotFound
            || (int)_transcientEnd != APNotFound;
}
