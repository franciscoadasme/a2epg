...
QObject *APController::currentObject()
{
	...
	Q_ASSERT_X(mappedIndex.row() >= 0 && mappedIndex.row() < count(), "APController::currentObject()", "Index out of range");
...
void APController::itemSelectionDidChange(const QItemSelection &selected, const QItemSelection &)
{
	if (selected.count() == 0) {
	...
	Q_ASSERT_X(selected.count() == 1, "APController::itemSelectionDidChange", "Multi-selection detected");

	QItemSelection selectionMappedToSource = proxyModel->mapSelectionToSource(selected);
...
{
	Q_ASSERT_X(index >= 0 && index < count(), "APController::objectAt()", "Index out of range");
...
QObject *APController::selectedObject()
{
	...
	Q_ASSERT_X(indices.count() == 1, "APController::remove()", "Multi-selection detected");
...
void APController::setCurrentObject(QObject *object)
{
	...
	selectionModel->setCurrentIndex(mappedIndex, QItemSelectionModel::ClearAndSelect | QItemSelectionModel::Rows);
}
...
void APController::setSelectedObject(QObject *object)
{
	...
	QModelIndex modelIndex = index(row, 0, QModelIndex());
	QModelIndex mappedIndex = proxyModel->mapFromSource(modelIndex);
	selectionModel->select(mappedIndex, QItemSelectionModel::ClearAndSelect | QItemSelectionModel::Rows);
}
