...
{
    APPanoramicWidget *panoramic = (APPanoramicWidget *)parentWidget();
    float horizontalMultiplier = (float)parentWidget()->width()
                                 / panoramic->numberOfPointsThatCanBeDrawn();
    uint pos = (geometry().left() + event->x()) / horizontalMultiplier;
    panoramic->setShouldUpdateOffset(false);
    APViewportHandler::goTo(panoramic->offset() + pos,
                            Qt::AlignLeft,
                            parentWidget());
}
...
void APPanoramicKnobWidget::updateKnobGeometry()
{
    ...
    APPanoramicWidget *panoramic = (APPanoramicWidget *)parentWidget();
    int numberOfPointsThatCanBeDrawn = panoramic->numberOfPointsThatCanBeDrawn();
    if ((int)viewport->offset() >= offset
        && (int)viewport->offset() <= offset + numberOfPointsThatCanBeDrawn) {
        float horizontalMultiplier = (float)parentWidget()->width()
                                     / numberOfPointsThatCanBeDrawn;

        EPSignalWidget *epsw = MainWindow::instance()->epsignalWidget();
        int numberOfPoints = epsw->numberOfPointsThatCanDraw();
...
void APPanoramicSignalWidget::mouseReleaseEvent(QMouseEvent *event)
{
    ...
    float horizontalMultiplier = (float)width() / numberOfPointsThatCanBeDrawn;
...
void APPanoramicSignalWidget::paintEvent(QPaintEvent *event)
{
    ...
    QRect innerRect = rect().adjusted(0, SignalVerticalPadding,
                                      0, -SignalVerticalPadding);
    float verticalMultiplier = (float)innerRect.height() / signal->amplitude();
    int verticalOffset = signal->maximum() * verticalMultiplier
                         + SignalVerticalPadding;
    ...
    float horizontalMultiplier = (float)width()
                                 / (float)numberOfPointsThatCanBeDrawn;
...
//        qDebug().nospace() << "APParonamicSignalWidget::paintEvent() => from="
//                           << from << " to=" << to;
        ...
        for (float i = from + 1; i < to
             ...
             && i < points.length(); i++) {
            // be aware that we multiply by -1 to flip out the signal, so
            // positive values point to top
...
void APPanoramicWidget::emitRangeText()
{
    ...
    int to = proposedNumberOfPoints > numberOfPoints
             ? numberOfPoints
             : proposedNumberOfPoints;
...
void APPanoramicWidget::goToNextSection()
{
    ...
    if (_offset + numberOfPointsInOneHour
        >= (int)EPSignalsController::activeSignal()->numberOfPoints())
