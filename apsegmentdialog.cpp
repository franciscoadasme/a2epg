...
void APSegmentDialog::accept()
{
	...
	_segment->setTranscientRange(transformSecondsToNumberOfPoints(ui->startSpinBox->value()),
								 transformSecondsToNumberOfPoints(ui->endSpinBox->value()));

	_segment->setType( APSegmentTypesController::typeAt(ui->typeComboBox->currentIndex()) );
	...
	if (!collidedSegments.isEmpty()) {
		...
		if (ret == QMessageBox::Save) {
			...
			ui->startSpinBox->setValue(transformNumberOfPointsToSeconds(_segment->start()));
			ui->endSpinBox->setValue(transformNumberOfPointsToSeconds(_segment->end()));
			_segment->resetTranscientValues();
			return; // if user does not accept, wait for him to fix collisions manually
		}
	} else {
		_segment->pushChanges();
	}
...
int APSegmentDialog::askAboutCollisions(EPSegment *segment, QList<EPSegment *> collidedSegments)
{
	QMessageBox msgBox(MainWindow::instance());
	msgBox.setWindowTitle("AutoEPG ~ Resolving collision");
	msgBox.setText(tr("<strong>%1 segment collision(s) was found.</strong>").arg(collidedSegments.length()));
	msgBox.setInformativeText("<strong>Note</strong>: segment collisions will be automatically fixed, provoking changes in "
							  "surrounding segments and sometimes, its removal.<br /><br />"
							  "Are you sure you want to keep your changes?");
	msgBox.setStandardButtons(QMessageBox::Save | QMessageBox::Discard);
	msgBox.setDefaultButton(QMessageBox::Discard);

	QString details = tr("Focused segment range is [ %1, %2 ] and it will be changed to [ %3, %4 ].\n\n"
						 "Affected segments:")
...
int APSegmentDialog::runForSegment(EPSegment *segment)
{
	...
	instance()->ui->endSpinBox->setMaximum( transformNumberOfPointsToSeconds(EPSignalsController::activeSignal()->numberOfPoints()) );
...
{
	ui->durationLabel->setText(tr("Duration: %1 second(s)").arg( ui->endSpinBox->value() - ui->startSpinBox->value() + 0.01 ));
}
